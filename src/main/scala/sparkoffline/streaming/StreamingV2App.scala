...
import sparkoffline.customsort.ProductClass
...
object StreamingV2App {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setMaster("local[2]")
      ...
      .set("spark.streaming.kafka.maxRatePerPartition", "100") // kafka限速
      .set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")  // 序列化开关设置为kryo
      .registerKryoClasses(Array(classOf[ProductClass]))  // 需要手工注册自己的类，使得该类使用kryo进行序列化。默认是使用java的序列化方式
